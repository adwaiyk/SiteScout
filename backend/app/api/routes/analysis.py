"""
SiteScout — Quick Site Analysis API Route (coordinate-only, no project required).

Endpoint:
  POST /api/analysis/scan-site — Run full analysis on arbitrary coordinates
"""
...
class SiteAnalysisRequest(BaseModel):
    """Quick-scan request — coordinates only, no project context."""
...
async def scan_new_site(request: SiteAnalysisRequest) -> dict:
    """
    Run a full multi-source analysis on arbitrary coordinates.

    This endpoint does NOT require project context and is useful for
    quick screening before committing a site to a project.
    """
    try:
        nasa_task = fetch_nasa_environmental_data(request.latitude, request.longitude)
        osm_task = fetch_osm_infrastructure(
            request.latitude, request.longitude, radius_m=10000
        )
