"""
SiteScout — Project & Site Management API Routes.

Endpoints:
  POST   /projects/                                 — Create project
  GET    /projects/                                 — List user's projects
  POST   /projects/{project_id}/sites               — Register a site
  POST   /projects/{project_id}/sites/{site_id}/analyze — Full site analysis
"""
...
def register_site(
    ...
    """Register a new site within a project."""
    project = (
        db.query(Project)
...
@router.post(
    "/{project_id}/sites/{site_id}/analyze",
...
async def analyze_and_save_site(
    ...
    """
    Run a full multi-source analysis on a registered site.

    Fetches NASA POWER climate data, OSM infrastructure proximity, and
    land-use conflict data concurrently, then generates solar/wind
    predictions and persists the results as a ScanLog.
    """
    # 1. Verify ownership
    project = (
        db.query(Project)
    ...
    site = (
        db.query(Site)
        .filter(Site.id == site_id, Site.project_id == project.id)
        .first()
    )
    if not site:
        raise HTTPException(
    ...
    nasa_data, osm_data, conflict_data = await asyncio.gather(
        ...
        fetch_osm_infrastructure(lat=latitude, lon=longitude),
    ...
    # 5. Compile master analysis payload
    full_analysis = {
        "site_name": site.name,
