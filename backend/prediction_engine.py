...
SOLAR_MODEL_PATH = os.path.join(MODELS_DIR, "solar_model.joblib")
WIND_MODEL_PATH = os.path.join(MODELS_DIR, "wind_model.joblib")

# Load models into memory on module import
...
def predict_solar_potential(
    ...
    """
    Predicts solar energy production using trained XGBoost Regressor model if available,
    otherwise falls back to heuristic computation.
    """
    ...
    return {
        ...
        "suitability": "Excellent" if capacity_factor > 18 else "Moderate" if capacity_factor > 14 else "Low"
    }
...
def predict_wind_potential(
    ...
    """
    Predicts wind energy production using trained XGBoost Regressor model if available,
    otherwise falls back to heuristic computation.
    """
    ...
    return {
        ...
        "capacity_factor_percent": round(capacity_factor, 2),
        "inference_engine": inference_source,
        "suitability": "Excellent" if capacity_factor > 30 else "Moderate" if capacity_factor > 20 else "Low"
    }