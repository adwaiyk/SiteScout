"""
SiteScout — ML Model Training Script.

Generates synthetic physics-informed training data and trains XGBoost
models for solar and wind energy prediction.
...
def generate_synthetic_dataset(samples: int = 5000):
    """
    Generate realistic historical training samples correlating climate metrics
    (irradiance, temperature, wind speed) with power output (MWh).
    """
    np.random.seed(42)
...
def train_solar_model(df: pd.DataFrame) -> None:
    """Train and save the solar XGBoost regressor."""
...
def train_wind_model(df: pd.DataFrame) -> None:
    """Train and save the wind XGBoost regressor."""
